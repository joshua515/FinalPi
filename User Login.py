...
def register():
    ...
    # set globals to be used across the program.
    global username
    global password
    ...
    Label(register_screen, text="Please enter details below\n Exit out of the window after completing registration.").pack()
...
def register_user():
    ...
    username_info = username_info.encode('utf-8')
    password_info = password_info.encode('utf-8')

    # encode user and password
    result1 = hashlib.sha1(username_info).hexdigest()
    result2 = hashlib.sha1(password_info).hexdigest()
...
def login_verify():
    ...
    hashed_pass = hashlib.sha1(password1).hexdigest()
